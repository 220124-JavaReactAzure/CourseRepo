...
**NOTE**: In order to minimize time dedicated to environment setup, it is VERY important that you install these technologies before training begins. It is also important that you install the correct versions of the tools listed here as not all versions of the technologies we will use are guaranteed to be compatible with each other. If you choose to use a different version of one of the tools listed here, you will be responsible for debugging issues that are specific to your version.

<hr>

## Required Technologies

The following technologies are required in order to complete this phase of the program. **Click on name to jump to scoop installation, manual link at bottom of scoop step**:

-   [**Git**](#step-3-install-git): We'll be using Git as a version control tool during the course of this program. Not only will you use Git to track changes to your projects, but you'll also use it to collaborate with your trainer and with your fellow associates.
-   [**Java SE Development Kit 8**](#step-4-install-java): We will be utilizing Java 8 as our primary language for the duration of the program. This software development kit will provide the compiler and runtime environment needed to quickly and efficiently develop Java applications.
-   [**Eclipse**](#step-5-install-eclipse): We will be using Eclipse as our _integrated development environment_ (IDE). Though you don't need an IDE to write code, an IDE provides several features that make rapid development (e.g. quickly writing and refactoring code) easier.
    -   **Note**: Feel free to use the IDE of your choice but you might be responsible for debugging issues that arise with that tool.
-   [**Apache Maven**](#step-6-install-maven): Maven is a software project management and build automation tool. We will use it to manage our projects' builds as it provides dependency management and streamlines several aspects of the testing life cycle.
-   [**PostgreSQL 10**](#step-10-install-postgresql): PostgreSQL is an open source relational database. We will leverage this technology in order to persist and organize data.
-   [**DBeaver**](#step-7-install-dbeaver): DBeaver is a free, open source universal database tool that is compatible with several external data sources, PostgreSQL included. We'll use DBeaver in order to modify our databases and persist data using a simple user interface provided to us by the tool.
-   [**Apache Tomcat 8**](#step-8-install-apache-tomcat): During this program, we will explore web development. As a result, we'll need a runtime environment for our web applications. Apache Tomcat provides a web server environment in which we can run our Java code so that we can easily and quickly deploy our web applications.
-   [**Postman**](#step-9-install-postman): Postman is used for testing RESTful APIs. We'll use it to set up collections of tests for the RESTful APIs we design throughout the program. It can also be used to quickly test a single endpoint.
-   [**Visual Studio Code**](#step-9-install-visual-studio-code-optional): Visual Studio Code is a text editor that makes viewing and modifying code simple. Do not confuse this tool with an integrated development environment as it does not come equipped with all of the development tools you typically find in an IDE.
-   [**Docker (cannot be installed with scoop)**](#step-10-install-docker): This is a open-source contairization platform. Enables developers to package applications into containers, standardized executable components combine the source code, libraries and dependicie required to run the application in any environment.
...
A _package manager_ is a command line tool that allows you to install and manage software. You can use a package manager to install all of the software listed here.
...
**_Didn't want to use scoop? Here is the:_**
[Manual Installation - Git](#step-1-install-git)
...
**_Didn't want to use scoop? Here is the:_**
[Manual Installation - Java](#step-2-install-java)
...
**_Didn't want to use scoop? Here is the:_**
[Manual Installation - Eclipse](#step-3-install-eclipse)
...
**_Didn't want to use scoop? Here is the:_**
[Manual Installation - Maven](#step-4-install-apache-maven)
...
**_Didn't want to use scoop? Here is the:_**
[Manual Installation - DBeaver](#step-6-install-dbeaver)
...
**_Didn't want to use scoop? Here is the:_**
[Manual Installation - Apache Tomcat](#step-7-install-apache-tomcat)
...
**_Didn't want to use scoop? Here is the:_**
[Manual Installation - Postman](#step-8-install-postman)
...
**_Didn't want to use scoop? Here is the:_**
[Manual Installation - PostgeSQL](#step-5-install-postgresql-10)
...
**_Didn't want to use scoop? Here is the:_**
[Manual Installation - VSCode](#step-9-install-visual-studio-code)
...
Go to [the docker website here to download docker desktop](https://www.docker.com/products/docker-desktop)
